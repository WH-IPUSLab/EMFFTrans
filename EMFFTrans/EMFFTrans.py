...
from EMFFTrans.LinearCrossAttention import LinearCrossAttention

logger = logging.getLogger(__name__)

class Channel_Embeddings(nn.Module):

    def __init__(self,embeddings_dropout_rate, n_patch_h,n_patch_w, img_h,img_w, in_channels):
        super().__init__()
        img_size = (img_h,img_w)
        patch_div_size = (n_patch_h,n_patch_w)
        n_patches = patch_div_size[0] * patch_div_size[1]
        patch_h_size = img_size[0] // patch_div_size[0]
        patch_w_size = img_size[1] // patch_div_size[1]

        self.patch_embeddings = nn.Sequential(Conv2d(in_channels=in_channels,out_channels=in_channels//8,kernel_size=(1,1), stride=(1,1)),
                                                Conv2d(in_channels=in_channels//8,out_channels=in_channels//8,kernel_size=(patch_h_size,patch_w_size), stride=(patch_h_size,patch_w_size)),
                                                Conv2d(in_channels=in_channels//8,out_channels=in_channels,kernel_size=(1,1), stride=(1,1)),
                                              )
...
class Attention_org(nn.Module):
    def __init__(self,  vis,channel_num,KV_size,num_heads,attention_dropout_rate):
...
class Block_ViT(nn.Module):
    def __init__(self,  vis, channel_num,expand_ratio, KV_size):
        ...
        #self.channel_attn = Attention_org(vis, channel_num,KV_size=KV_size,num_heads=8,attention_dropout_rate=0.1)
        self.channel_attn = LinearCrossAttention( channel_num,attention_dropout_rate=0.1)
        ...
        self.ffn1 = Mlp(channel_num[0],channel_num[0]*expand_ratio,dropout_rate=0)
        self.ffn2 = Mlp(channel_num[1],channel_num[1]*expand_ratio,dropout_rate=0)
        self.ffn3 = Mlp(channel_num[2],channel_num[2]*expand_ratio,dropout_rate=0)
        self.ffn4 = Mlp(channel_num[3],channel_num[3]*expand_ratio,dropout_rate=0)
...
class Encoder(nn.Module):
    def __init__(self, num_layers, vis, channel_num):
        ...
        for _ in range(num_layers):
            layer = Block_ViT( vis, channel_num,expand_ratio=4,KV_size=240)
...
class EffAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q_all = torch.split(q, math.ceil(N), dim=-2)
        k_all = torch.split(k, math.ceil(N), dim=-2)
        v_all = torch.split(v, math.ceil(N), dim=-2)
        output = []
...
class EMFFTrans(nn.Module):
    def __init__(self,  vis = False, img_h=360,img_w=480, embeddings_dropout_rate = 0.1, num_layers=1, channel_num=[16, 32, 64, 128],n_patch_h = 3, n_patch_w = 3):
        super().__init__()

        self.n_patch_h = n_patch_h
        self.n_patch_w = n_patch_w
        self.patch_h_size = img_h // n_patch_h
        self.patch_w_size = img_w // n_patch_w

        self.embeddings_1 = Channel_Embeddings(embeddings_dropout_rate, self.n_patch_h, self.n_patch_w, img_h=img_h//2, img_w=img_w//2,    in_channels=channel_num[0])
        self.embeddings_2 = Channel_Embeddings(embeddings_dropout_rate, self.n_patch_h, self.n_patch_w, img_h=img_h//4, img_w=img_w//4, in_channels=channel_num[1])
        self.embeddings_3 = Channel_Embeddings(embeddings_dropout_rate, self.n_patch_h, self.n_patch_w, img_h=img_h//8, img_w=img_w//8, in_channels=channel_num[2])
        self.embeddings_4 = Channel_Embeddings(embeddings_dropout_rate, self.n_patch_h, self.n_patch_w, img_h=img_h//16,img_w=img_w//16, in_channels=channel_num[3])
        self.encoder = Encoder(num_layers, vis, channel_num)

        self.reconstruct_1 = Reconstruct(channel_num[0], channel_num[0], kernel_size=1,scale_factor=(self.patch_h_size//2,self.patch_w_size//2))
        self.reconstruct_2 = Reconstruct(channel_num[1], channel_num[1], kernel_size=1,scale_factor=(self.patch_h_size//4,self.patch_w_size//4))
        self.reconstruct_3 = Reconstruct(channel_num[2], channel_num[2], kernel_size=1,scale_factor=(self.patch_h_size//8,self.patch_w_size//8))
        self.reconstruct_4 = Reconstruct(channel_num[3], channel_num[3], kernel_size=1,scale_factor=(self.patch_h_size//16,self.patch_w_size//16))
    ...
    def forward(self,en1,en2,en3,en4):
        ...
        x1 =  self.resize_output_if_needed(x1,180,240)
        x2 =  self.resize_output_if_needed(x2,90,120)
        x3 =  self.resize_output_if_needed(x3,45,60)
        x4 =  self.resize_output_if_needed(x4,22,30)
